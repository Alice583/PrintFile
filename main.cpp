...
void FromStartToEnd(ArgumentsSTD* argumentsStd) {
    ...
    if (!filename_for_open) {
        std::wcerr << L"Файл не открыт или введен неверно" << "\n";
    } else {
...
char getDelimiter(char* args){
    ...
    if (strlen(args) >= 2){
        std::wcerr << L"Ошибка, неверный делиметр" << '\n';
    }
    return 0;
}


const char* kLinesStr = "--lines=";
const char* kDelimiterStr = "--delimiter=";
const char* kTailStr = "--tail=";


ArgumentsSTD CheckCorrectness(int argc, char* argv[], ArgumentsSTD argumentsStd) {
    setlocale(LC_ALL, "rus");
    if (argc < 2) {
        std::wcerr << L"Извините, нужно ввести аргументы и имя файла" << "\n";
    } else {
        for (int i = 1; i < argc; i++) {
            if (std::strcmp(argv[i], "-l") == 0){
                argumentsStd.lines = std::stoll(argv[i + 1]);
                i++;
            } else if (std::strncmp(argv[i], kLinesStr, strlen(kLinesStr)) == 0){
                argumentsStd.lines = std::stoll(argv[i] + 8);
            } else if (std::strcmp(argv[i], "-t") == 0) {
                argumentsStd.from_tail = true;
            } else if (std::strncmp(argv[i], kTailStr, strlen(kTailStr)) == 0){
                argumentsStd.from_tail = true;
            } else if  (std::strcmp(argv[i], "-d") == 0) {
                argumentsStd.delimiter = getDelimiter(argv[i + 1]);
                i++;
            } else if (std::strncmp(argv[i], kDelimiterStr, strlen(kDelimiterStr)) == 0) {
